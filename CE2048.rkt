...
; ------------------------------------------------- GetTileFrom2DCoords
(define (GetTileFrom2DCoords Current_X Current_Y X)
    (+ Current_X (* X Current_Y)))

; ------------------------------------------------- GetCurrentXFromCurrentTile
(define (GetCurrentXFromCurrentTile CurrentTile X)
    (remainder CurrentTile X))
    
; ------------------------------------------------- GetCurrentYFromCurrentTile
(define (GetCurrentYFromCurrentTile CurrentTile X)
    (quotient CurrentTile X))


; Print a matrix. For Showing the game in console.
(define (ShowMatrix matrix)
    (cond
        ; Stop condition.
        [(empty? matrix) (displayln "")]
        ; Advance to the next row if the current one is empty
        [(empty? (car matrix)) (displayln "") (ShowMatrix (cdr matrix))]
        [else (display "\t|") (display (caar matrix)) (display "|\t") (ShowMatrix (cons (cdar matrix) (cdr matrix)))]
        ))
...
; ; (GameLoop Board Difficulty seed X Y CurrentX CurrentY (read-char))

; (define (GameLoop Board Difficulty seed X Y CurrentX CurrentY Key MovementCount FlushReadChar CheckWinCondition)

;     (newline)
;     (newline)

;     ; ; Show MovementCount DEBUG
;     ; (display "---------Enters GameLoop ")
;     ; (display MovementCount)
;     ; (display " with '")
;     ; (display Key)
;     ; (displayln "'---------")
...
;     ; Print board
;     (cond [(not FlushReadChar) (ShowBoard Board 0 X (GetTileFrom2DCoords CurrentX CurrentY X))])

;     ; Wait and Act based on user
;         (cond
;         [(and CheckWinCondition (WinCondition Board 0 seed Difficulty)) (displayln "You Win!")]   ; Checks wind condition if the previous 
;         [(char=? Key #\space)
;             ; (displayln "Pressed Space")
;             (cond[(= (IsMine seed CurrentX CurrentY X Difficulty) 1) (displayln "Game Over!")]
;                 [else (GameLoop (RevealTile CurrentX CurrentY X Y seed Difficulty Board) Difficulty seed X Y CurrentX CurrentY (read-char) (+ MovementCount 1) #false #true)])]
        
;         [(char=? Key #\w)
;             ; (displayln "Pressed w")
;             (GameLoop Board Difficulty seed X Y CurrentX (modulo (- CurrentY 1) Y) (read-char) (+ MovementCount 1) #false #false)]
;         [(char=? Key #\a)
;             ; (displayln "Pressed a")
;             (GameLoop Board Difficulty seed X Y (modulo (- CurrentX 1) X) CurrentY (read-char) (+ MovementCount 1) #false #false)]
;         [(char=? Key #\s)
;             ; (displayln "Pressed s")
;             (GameLoop Board Difficulty seed X Y CurrentX (modulo (+ CurrentY 1) Y) (read-char) (+ MovementCount 1) #false #false)]
;         [(char=? Key #\d)
...
;         [(char=? Key #\q)
;             ; (displayln "Pressed q")
;             (GameLoop (ToggleFlagged Board '() 0 (GetTileFrom2DCoords CurrentX CurrentY X)) Difficulty seed X Y CurrentX CurrentY (read-char) (+ MovementCount 1) #false #false)]


;         [else
;         ; (displayln (string-append "Other key pressed: " (string Key)))
;         (GameLoop Board Difficulty seed X Y CurrentX CurrentY (read-char) (+ MovementCount 1) #true #false)])
...
(define (GameLoop matrix FlushReadChar CheckWinCondition Key)



    ; Print board
    (cond [(not FlushReadChar)
        (newline)
        (display "\t-----------------------------------------------------------------------------------\t\t")
        (newline)
        (ShowMatrix matrix)])
    ...
    ; Wait and Act based on user
        ...
        [(char=? Key #\w)
            ; (displayln "Pressed w")
            (GameLoop (InsertRandomNumber(MoveUpMatrix matrix)) #f CheckWinCondition (read-char))]
        [(char=? Key #\a)
            ; (displayln "Pressed a")
            (GameLoop (InsertRandomNumber(MoveLeftMatrix matrix)) #f CheckWinCondition (read-char))]
        [(char=? Key #\s)
            ; (displayln "Pressed s")
            (GameLoop (InsertRandomNumber(MoveDownMatrix matrix)) #f CheckWinCondition (read-char))]
        [(char=? Key #\d)
            ; (displayln "Pressed d")
            (GameLoop (InsertRandomNumber(MoveRightMatrix matrix)) #f CheckWinCondition (read-char))]

        ; [(char=? Key #\q)
        ;     ; (displayln "Pressed q")
        ;     (GameLoop (InsertRandomNumber(MoveMatrix matrix)) #f CheckWinCondition (read-char))]


        [else
        ; (displayln (string-append "Other key pressed: " (string Key)))
        (GameLoop matrix #t CheckWinCondition (read-char))])
...
; Test (GenerateRow current_tile x row)
; (GenerateRow 0 4 '())

; (GenerateMatrix 12 0 3 4 '())

; (FindSuccesfulMatches '(4 2 0 2 2) '() 0)
; (MoveLeftRowAux '(4 2 0 2 2) (FindSuccesfulMatches '(4 2 0 2 2) '() 0) #f 0)
; ; (FindSuccesfulMatches '(0 0 0 4 0 0 0 2 0 0 0 2 0 2 0 0 0) '() 0)
; ; (MoveLeftRowAux '(0 0 0 4 0 0 0 2 0 0 0 2 0 2 0 0 0) (FindSuccesfulMatches '(0 0 0 4 0 0 0 2 0 0 0 2 0 2 0 0 0) '() 0) #f)
; (ZerosList '() 2)

; (MoveLeftRow '(0 2 0 2 0 0 4 0 0 2 0 0 4 0 2 0 8 8 8 16))
; (MoveRightRow '(0 2 0 2 0 0 4 0 0 2 0 0 4 0 2 0 8 8 8 16))
